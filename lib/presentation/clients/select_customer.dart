      ...
      case 'priceListSelect':
        return Container(
          ...
          child: DropdownButtonFormField<int>(
            ...
            items: dataList
                .where((listPrice) =>
                    listPrice['m_pricelist_id'] is int &&
                    listPrice['price_list_name'] != '')
                .map<DropdownMenuItem<int>>((list) {
              print('Lista $list');
              return DropdownMenuItem<int>(
                value: list['m_pricelist_id'] as int,
                child: SizedBox(
                  width: mediaScreen * 0.7,
                  child: Text(
                    list['price_list_name'] as String,
                    style: const TextStyle(
                        fontFamily: 'Poppins Regular',
            ...
            onChanged: (newValue) {
              print('esto es la lista de precios completa ${dataList}');
              String namePriceList =
                  invoke('obtenerNombrePriceList', newValue, dataList);
              print("esto es el nombre de impuesto $namePriceList");
              onSelected(newValue, namePriceList);
            },
            decoration: InputDecoration(
                errorStyle: const TextStyle(fontFamily: 'Poppins Regular'),
                contentPadding:
            ...
            validator: (value) {
              if (value == null || value == 0) {
                return 'Por favor selecciona una Lista de Precio';
              }
              return null;
            },
          ),
      ...
      case 'selectState':
        return Container(
          ...
          child: DropdownButtonFormField<int>(
            ...
            items: dataList
                .where((state) =>
                    state['c_region_id'] is int && state['name'] != '')
                .map<DropdownMenuItem<int>>((country) {
              print('tax $country');
              return DropdownMenuItem<int>(
                value: country['c_region_id'] as int,
                child: SizedBox(
            ...
            onChanged: (newValue) {
              print('esto es el State List ${dataList}');
              String nameCountry =
                  invoke('obtenerNombreState', newValue, dataList);
              print("esto es el nombre del Estado $nameCountry");
      ...
      case 'selectTypeAccountBank':
        return Container(
          ...
          child: DropdownButtonFormField<int>(
            ...
            items: dataList
                .where((bankAccount) =>
                    bankAccount['c_bank_account_id'] is int &&
                    bankAccount['bank_name'] != '')
                .map<DropdownMenuItem<int>>((bankAcc) {
              print('bankAccount $bankAcc');
              return DropdownMenuItem<int>(
                value: bankAcc['c_bank_account_id'] as int,
                child: SizedBox(
                    width: mediaScreen * 0.7,
                    child: Text(bankAcc['c_bank_account_id'] != 0 ? '${bankAcc['account_no'].toString().substring(bankAcc['account_no'].toString().length - 4)} - ${bankAcc['bank_name']}': '${bankAcc['bank_name']}',
                      style: const TextStyle(fontFamily: 'Poppins Regular'),
                    )),
              );
            }).toList(),
